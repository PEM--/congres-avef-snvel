...
log = Logger.createLogger 'SSR PDF'
...
class @PdfRenderer extends PDFDocument
  ...
  formatter: (innerData) ->
    ...
    log.warn 'Unmanaged data type', innerData, typeof innerData
    return null
  ###*
...
domino = Npm.require 'domino'
SD.Utils.cgvPdf = null

SD.Utils.updateCgv = (page) ->
  pdf = new PdfRenderer
  pdf.h1 page.title
  content = marked page.content
  dom = (domino.createWindow content).document
  for idx in [0...dom.body.children.length]
    node = dom.body.childNodes[idx]
    # Skip empty markdown for prettyfication
    continue if node._data is '\n' and node.tagName is undefined
    pdf[node.tagName?.toLowerCase()]? s.stripTags node.innerHTML
  buffer = pdf.outputSync()
  SD.Utils.cgvPdf = buffer.toString 'base64'

log.info 'Setting up CGV PDF'
# Create the initial CGV as PDF
SD.Utils.updateCgv(SD.Structure.basicPages.collection.findOne({url: 'cgv'}))
log.info 'CGV PDF generated'
# Update CGV PDF when the BasicPage changes
SD.Structure.basicPages.collection.find().observeChanges
  'changed': (id, fields) ->
    page = SD.Structure.basicPages.collection.findOne id
    if page.url is 'cgv'
      SD.Utils.updateCgv page
      log.info 'CGV PDF updated'
